...
export default function FAQGroup() {
  ...
  const [isModalVisibleDetail, setIsModalVisibleDetail] =
    React.useState<boolean>(false);
  const [tmpData, setTmpData] = React.useState<any>(initialFaqs);
  ...
  const menu = (record: FAQSProps) => (
    <Menu onClick={(e) => handleClickAction(record, e.key)}>
      <Menu.Item key="edit">Edit FAQ Group</Menu.Item>
      <Menu.Item key="delete">Delete FAQ Group</Menu.Item>
    </Menu>
  );

  const handleClickAction = (props: any, key: string) => {
    if (key === 'detail') {
      navigate(`/news/`);
    } else if (key === 'edit') {
      navigate(`/faq-group/${props.faqGroupId}/edit`);
    } else if (key === 'delete') {
      setTmpData(props);
  ...
  const handleStatusChange = async () => {
    try {
      setIsLoadingUpdateStatus(true);
      await httpRequest.patch('/faq-groups/' + tmpData.faqId, {
  ...
  return (
    <>
        ...
        rightAction={
          <Space>
            <Button
              ...
              type="primary"
              onClick={() => navigate('/faq-group/add')}
            >
              Add FAQ Group
            </Button>
          </Space>
      ...
      >
        <p>
          Are you sure want to delete question <b>"{tmpData.groupName}"</b> from
          FAQ Group?
        </p>
      </Modal>
